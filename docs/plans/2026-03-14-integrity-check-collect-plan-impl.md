...
class DataIntegrityReportRefreshView(APIView):
    ...
    def post(self, request, pk):
        ...
        if report.status == 'GENERATING':
            return Response({'error': '报告正在生成中'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({'success': True, 'data': DataIntegrityReportSerializer(report).data})
```
...
const currentFilters = ref<{ data_type?: string; stock_code?: string; status?: string }>({})
...
const columnDefs = [
  ...
  },
  {
    ...
    cellRenderer: (params: any) => {
      const isFixed = params.data?.status === 'FIXED'
      return isFixed 
        ? '<span style="color: #67c23a;">✓ 已修复</span>' 
        : '<span style="color: #909399;">⏳ 待修复</span>'
    }
  },
  {
    field: 'data_type',
...
const onFilterChanged = (event: any) => {
  ...
  if (filterModel.status_display) {
    currentFilters.value.status = filterModel.status_display.filter === '已修复' ? 'FIXED' : 'PENDING'
  }
  loadItems()
}
...
const refreshReport = async () => {
  ...
  try {
    await api.post(`/integrity-reports/${props.id}/refresh/`)
...
```python
class DataBrowseByTypeView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request, data_type):
        data_type_mapping = {
            'historical-quotes': HistoricalQuote,
        ...
        if not model:
            return Response({'error': '不支持的数据类型'}, status=404)
        ...
        stock_code = request.query_params.get('stock_code')
        if stock_code:
            queryset = queryset.filter(symbol__icontains=stock_code)
        ...
        serializer = self._get_serializer(model, page)
        return paginator.get_paginated_response(serializer.data)

    def _get_serializer(self, model, data):
        # 根据模型返回对应的序列化器
        serializer_map = {
            HistoricalQuote: HistoricalQuoteSerializer,
            BalanceSheet: BalanceSheetSerializer,
            # ... 其他模型
        }
        serializer_class = serializer_map.get(model)
        return serializer_class(data, many=True).data if serializer_class else []


class StockDataBrowseView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request, symbol, data_type):
        # 获取单个股票的指定类型数据
        pass
```
...
```python
path('data-browse/<str:data_type>/', views.DataBrowseByTypeView.as_view(), name='data-browse-type'),
path('stocks/<str:symbol>/<str:data_type>/', views.StockDataBrowseView.as_view(), name='stock-data-browse'),
```
...
```bash
git add backend/saa_collector/views.py backend/saa_collector/urls.py
git commit -m "feat: add data browse API endpoints"
```

---

## Task 14: 创建数据浏览 - 按股票页面

**Files:**
- Create: `frontend/src/views/DataBrowseStockView.vue`
  ...
  <div class="data-browse-stock">
    <el-card>
      <template #header>
        <div class="card-header">
          <span>数据浏览 - 按股票</span>
          <el-button link @click="$router.push('/data-browse/type')">
            切换到按类型浏览
          </el-button>
        </div>
      </template>
      ...
      <el-container>
        <el-aside width="300px" class="stock-list-aside">
          <el-input
        ...
        <el-main>
          ...
          <div v-else>
            <el-tabs v-model="activeDataType" @tab-change="loadData">
              <el-tab-pane
...
```bash
git add frontend/src/views/DataBrowseStockView.vue
git commit -m "feat: add DataBrowseStockView for browsing data by stock"
```

---

## Task 15: 创建数据浏览 - 按类型页面

**Files:**
- Create: `frontend/src/views/DataBrowseTypeView.vue`
  ...
  <div class="data-browse-type">
    <el-card>
      <template #header>
        <div class="card-header">
          <span>数据浏览 - 按类型</span>
          <el-button link @click="$router.push('/data-browse/stock')">
            切换到按股票浏览
          </el-button>
        </div>
      </template>
      ...
      <div class="filter-bar">
        <el-select v-model="selectedDataType" @change="loadData" style="width: 200px;">
          <el-option
...
```bash
git add frontend/src/views/DataBrowseTypeView.vue
git commit -m "feat: add DataBrowseTypeView for browsing data by type"
```
...
<el-menu-item index="/integrity-check">
  <el-icon><DocumentChecked /></el-icon>
  <span>完整性检查</span>
</el-menu-item>
<el-menu-item index="/collect-manage">
...
git commit -m "feat: update navigation structure with data browse"
```
...
5. **执行机制** - 支持并行/顺序执行
6. **修复反馈** - 计划完成后自动更新报告状态
7. **数据浏览** - 按股票/按类型两种浏览方式
