  ...
  if (dataType === 'info') {
    for (let i = 0; i < count; i++) {
      ...
      rows.push({
        ...
        listing_date: new Date(Date.now() - Math.random() * 10 * 365 * 24 * 3600000).toISOString().split('T')[0] ?? '',
        delisting_date: '2200-01-01',
  ...
  'saa_stocks': {
    ...
    config: {
      fields: [
        ...
        { name: 'listing_date', label: '上市日期', visible: true, order: 5, width: 110, format: 'date' },
        { name: 'delisting_date', label: '退市日期', visible: true, order: 6, width: 110, format: 'date' },
